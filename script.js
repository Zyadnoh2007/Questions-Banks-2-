...
document.addEventListener("DOMContentLoaded", () => {
    ...
    loadNewsTicker(); // 📢 تحميل شريط الأخبار
});

// --- 📢 إدارة شريط الأخبار (جديد) ---
window.updateNewsTicker = function() {
    ...
    if(text) {
        ...
        loadNewsTicker();
        alert('تم تحديث شريط الأخبار ✅');
    } else {
        localStorage.removeItem('siteNews');
        loadNewsTicker();
        alert('تم إخفاء شريط الأخبار');
    }
};
...
function loadScript(src, cb, errCb) { if(loadedScripts[src]) { cb(); return; } const s = document.createElement('script'); s.src = src; s.onload = () => { loadedScripts[src]=true; cb(); }; s.onerror = errCb; document.head.appendChild(s); }        renderCustomQuizzesList();
...
function displayQuestion() {
    ...
    if (qData.hint) {
        ...
        <div class="hint-container">
            <button class="hint-btn" onclick="this.nextElementSibling.style.display='block';this.style.display='none'">💡 تلميح</button>
