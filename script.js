...
window.saveStudentName = function() {
    ...
    if (nameInput.length < 3) { errorMsg.style.display = "block"; return; }
...
window.checkAdminPassword = function() {
    ...
    if(pass === "admin123") {
        ...
        renderCustomQuizzesList(); // تحميل قائمة الحذف
    } else {
...
function displayQuestion() {
    ...
    if (qData.hint) {
        ...
        <div class="hint-container">
            <button class="hint-btn" onclick="this.nextElementSibling.style.display='block';this.style.display='none'">
                💡 تلميح
            </button>
...
// --- Admin Management (Render List & Delete) ---
function renderCustomQuizzesList() {
    ...
    if(list.length === 0) {
        container.innerHTML = '<p style="text-align:center; color:gray; width:100%;">لا توجد امتحانات مضافة يدوياً.</p>';
        return;
    }
...
function parseQuestionsText(text) {
    ...
    lines.forEach(l => {
        ...
        if(l.match(/^(Q\d+|س\d+|\d+)[:.)]/i) || l.includes('?')) {
            ...
            cur = { q: l.replace(/^(Q\d+|س\d+|\d+)[:.)]\s*/i, ''), options: [], a: 0, type: 'mcq' };
...
window.loadQuizSource = function(source) {
    ...
    // هنا بيحصل الدمج بين الملفات والكويزات اليدوية
    const scriptPath = `questions/${currentSubject}/${source}.js`;
    let fileQuizzes = {};
...
function displayQuestion() {
    ...
    q.options.forEach((opt, i) => {
        const isSelected = uAns && uAns.answer === i;
        optionsHtml += `<button class="answer-btn ${isSelected?'selected':''}" onclick="selectAnswer(${i})">${opt}</button>`;
    });
    optionsHtml += '</div>';
...
window.selectAnswer = function(i) {
    userAnswers[currentQuestionIndex] = { answer: i, isCorrect: i === currentQuiz[currentQuestionIndex].a };
    displayQuestion();
};
...
window.showReview = function() {
    ...
    currentQuiz.forEach((q, i) => {
        const ua = userAnswers[i];
        const correct = ua && ua.isCorrect;
        const uText = ua ? q.options[ua.answer] : "لم يجب";
        c.innerHTML += `
            <div class="review-question">
                <div style="font-weight:bold;">س ${i+1}: ${q.q}</div>
                <div class="review-option ${correct?'correct':'user-incorrect'}">إجابتك: ${uText}</div>
                ${!correct ? `<div class="review-option correct">الصحيح: ${q.options[q.a]}</div>` : ''}
            </div>`;
    });
    document.getElementById('results').style.display = 'none';
    document.getElementById('review-container').style.display = 'block';
