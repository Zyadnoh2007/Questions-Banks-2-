...
let currentSubject = 'microbiology'; // Default
let currentSource = '';
let currentQuizData = {};
let currentQuiz = [];
...
// تخزين المواد والمصادر المضافة يدوياً فقط
let customConfig = {
...
document.addEventListener("DOMContentLoaded", () => {
    // 1. دمج المواد والمصادر (الثابت + الجديد)
    renderAppUI();
...
function renderAppUI() {
    ...
    // تنظيف العناصر "الديناميكية" فقط (التي لها كلاس dynamic-item) لعدم تكرارها
    // ونترك العناصر الثابتة (اللي كتبناها في HTML) زي ما هي
    document.querySelectorAll('.dynamic-item').forEach(e => e.remove());

    // 1. إضافة المواد الجديدة
    ...
    // 3. تحديث قوائم الأدمن (يجمع كل المواد: الثابتة + الجديدة)
    updateAdminSelects();
    renderSettingsLists();
}

function updateAdminSelects() {
    // تجميع كل المواد من الـ DOM (عشان نجيب الثابت والجديد)
    const allSubjects = [];
    document.querySelectorAll('#main-nav .tab-btn').forEach(btn => {
        // نستخرج الـ ID من الـ onclick attribute
        const onclickText = btn.getAttribute('onclick');
        const id = onclickText.match(/'([^']+)'/)[1];
        allSubjects.push({ id: id, name: btn.textContent });
    });

    // تجميع كل المصادر
    const allSources = [];
    // المصادر الثابتة (من الكود)
    allSources.push({id:'bank', name:'بنك الأسئلة'}, {id:'doctor', name:'كويزات الدكتور'});
    // المصادر الجديدة
    ...
    srcSelect.innerHTML = allSources.map(s => `<option value="${s.id}">${s.name}</option>`).join('');
}

// --- ⚙️ إدارة الإعدادات (إضافة فقط) ---
// الحذف هيكون للمواد المضافة يدوياً فقط عشان الثابت ميبوظش
window.addNewSubject = function() {
    ...
    if(exists) return alert('المادة موجودة بالفعل');
...
function renderSettingsLists() {
    ...
    // عرض العناصر القابلة للحذف فقط (الجديدة)
...
window.checkAdminPassword = function() {
    ...
    if(pass === "admin123") {
        ...
        // تحديث القوائم للتأكد إن كل المواد ظاهرة
        updateAdminSelects();
    } else {
...
function renderCustomQuizzesList() {
    ...
    list.forEach(q => {
        // محاولة جلب الاسم من الثابت أو الديناميكي
        let subName = q.subject;
        // بحث في الثابت (بسيط)
        const staticBtn = document.querySelector(`#main-nav .tab-btn[onclick*="${q.subject}"]`);
        if(staticBtn) subName = staticBtn.textContent;
        // بحث في الديناميكي
        const dynSub = customConfig.subjects.find(s => s.id === q.subject);
        if(dynSub) subName = dynSub.name;

        let srcName = q.source === 'bank' ? 'بنك الأسئلة' : (q.source === 'doctor' ? 'كويزات الدكتور' : q.source);
        const dynSrc = customConfig.sources.find(s => s.id === q.source);
        if(dynSrc) srcName = dynSrc.name;
...
window.selectSubject = function(subject) {
    ...
    // بحث عن الزر (سواء ثابت أو ديناميكي)
...
function loadScript(src, cb, errCb) { if(loadedScripts[src]) { cb(); return; } const s = document.createElement('script'); s.src = src; s.onload = () => { loadedScripts[src]=true; cb(); }; s.onerror = errCb; document.head.appendChild(s); }        return;
    }
